...
target = target.sort_values(['M', 'V'], ascending=[True, True])
print(target.shape)
print(target)


# Count the number of groups having same X, Y
groups = target.groupby(['M', 'V'])
...
new_target.to_csv('new_train_target_MV.csv', index=False)
...
print(new_features)
new_features.to_csv('new_train_features_MV.csv', index=False)
